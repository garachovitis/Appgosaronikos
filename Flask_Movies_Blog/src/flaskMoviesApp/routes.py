...
from flaskMoviesApp.models import Beach
...
def delete_movie(movie_id):
    ...
    return redirect(url_for("root"))
#my add-------------------------------------------
##gemeise to get_activities
def activities(request):
    location = request.path_info.split('/')[-1]  # Αυτό θα πάρει το τελευταίο μέρος του URL
    return render_template (request, 'activities.html', {'location': location.capitalize()})

def get_activities_for_island(island):
    # This is a placeholder function that returns a list of dummy activities
    # Replace this with your actual function once your database is ready
    return [
...
def activities(island):
    # Use the island to determine what activities to display
    # You can use a database query or a dictionary to map islands to activities
    activities = get_activities_for_island(island)  # Replace this with your actual function

    return render_template('activities.html', activities=activities,location=island.capitalize())

def transport(request):
    location = request.path_info.split('/')[-1]  # Αυτό θα πάρει το τελευταίο μέρος του URL
    return render_template (request, 'transport.html', {'location': location.capitalize()})

@app.route('/transport/<island>')
def transport(island):
    return render_template('transport.html', island=island, location=island.capitalize())

def get_beach_data():
    session = db.session()
    beach_names = session.query(Beach.beachName).all()
    session.close()
    return [name[0] for name in beach_names]

@app.route('/beachData')
def beach_data():
    beach_data = get_beach_data()  # Replace this with the actual way you retrieve data from the database
...
def beaches(request):
    location = request.path_info.split('/')[-1]  # Αυτό θα πάρει το τελευταίο μέρος του URL
    return render_template (request, 'beaches.html', {'location': location.capitalize()})

@app.route('/beaches/<island>')
def beaches(island):
    return render_template('beaches.html', island=island, location=island.capitalize())
